...
with st.container():
    st.markdown('## Quantidade de Restaurantes por Países')
    df_aux_1 = df1.loc[:, ['Restaurant ID', 'Country Name']].groupby('Country Name').count().sort_values('Restaurant ID', ascending=False).reset_index().head(5)#.iloc[0, 0]
    fig = px.bar(df_aux_1, x='Country Name', y='Restaurant ID')
    st.plotly_chart(fig, use_container_width = True)

with st.container():
    st.markdown('## Quantidade de Cidades por Países')
...
with st.container():
    ...
    with col1:
        ...
        df_aux_7 = df1.groupby('Country Name')['Votes'].agg(['count','mean']).sort_values('mean', ascending=False).reset_index().head(5)
        fig = px.bar(df_aux_7, x='Country Name', y='mean')
        st.plotly_chart(fig, use_container_width = True)
    with col2:
        st.markdown('## Média de preço de um prato para 2 por país')
        df_aux = df1.groupby('Country Name')['Average Cost for two'].mean().sort_values(ascending=False).reset_index().head(5)
